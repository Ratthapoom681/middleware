...
def test_full_pipeline_cycle(config):
    """Test a complete DefectDojo polling cycle with mocked API responses."""
...
def test_run_cycle_skips_wazuh_polling(config):
    ...
    assert result["filtered"] == 0
    assert not hasattr(pipeline.wazuh, "fetch_alerts")
    pipeline.close()
